...
--- andrenalina
--- ----------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("adrenaline:Revive")
AddEventHandler("adrenaline:Revive",function(Health)
	exports["survival"]:Revive(Health or 70,false)
end)

local AdrenalineBlock = false
local AdrenalineStage = 0
local AdrenalineStart = 0

RegisterNetEvent("adrenaline:Block")
AddEventHandler("adrenaline:Block",function(Status)
	local Ped = PlayerPedId()

	if Status then
		AdrenalineBlock = true
		AdrenalineStage = 1
		AdrenalineStart = GetGameTimer()
		...
		TaskPlayAnim(Ped,"amb@medic@standing@kneel@enter","enter",8.0,8.0,1200,0,0,false,false,false)
	else
		AdrenalineBlock = false
		AdrenalineStage = 0
		AdrenalineStart = 0
...
CreateThread(function()
	while true do
		if AdrenalineBlock then
			...
			if AdrenalineStage == 1 and GetGameTimer() - AdrenalineStart >= 1200 then
				AdrenalineStage = 2
			end

			if AdrenalineStage == 2 then
				if not IsEntityPlayingAnim(Ped,"mini@cpr@char_a@cpr_str","cpr_pumpchest",3) then
