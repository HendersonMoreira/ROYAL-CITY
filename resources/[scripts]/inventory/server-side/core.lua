...
exports("IsClonePlate",function(Plate)
	return ClonePlates[Plate] ~= nil
end)

exports("GetClonePlateData",function(Plate)
	return ClonePlates[Plate]
end)

exports("GetOriginalPlate",function(Plate)
	if ClonePlates[Plate] then
		return ClonePlates[Plate]["OriginalPlate"]
	end

	return Plate
end)

exports("ApplyClonePlate",function(Passport,Model,OriginalPlate,NewPlate,Network)
	local Vehicle = NetworkGetEntityFromNetworkId(Network)
	if Vehicle and DoesEntityExist(Vehicle) then
		ClonePlates[NewPlate] = {
			["Passport"] = Passport,
			["Vehicle"] = Model,
		...
		Entity(Vehicle)["state"]:set("ClonePlate",true,true)
		Entity(Vehicle)["state"]:set("OriginalPlate",OriginalPlate,true)
...
exports("RestoreClonePlate",function(Plate,Network)
	if ClonePlates[Plate] then
		local Data = ClonePlates[Plate]
		local Vehicle = NetworkGetEntityFromNetworkId(Network)

		if Vehicle and DoesEntityExist(Vehicle) then
			SetVehicleNumberPlateText(Vehicle,Data["OriginalPlate"])
			Entity(Vehicle)["state"]:set("ClonePlate",false,true)
			Entity(Vehicle)["state"]:set("OriginalPlate",nil,true)
		end

		ClonePlates[Plate] = nil
		return Data["OriginalPlate"]
	end
...
function Creative.Use(Slot,Amount)
	...
	if Passport and not Active[Passport] then
		...
		elseif Use[Item] and ItemTypeCheck(Full,"Consumível") then
			print("Usando item consumível: " .. Item)
...
AddEventHandler("inventory:StealTrunk",function(Entity)
	...
	if Passport and not Active[Passport] then
		...
		if ClonePlates[Plate] then
			TriggerClientEvent("Notify",source,"Aviso","Não é possível acessar o porta-malas de um veículo clonado.","amarelo",5000)
			return false
		end
