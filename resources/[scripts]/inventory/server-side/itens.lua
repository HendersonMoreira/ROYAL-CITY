...
-- LOG DE ADRENALINA
-----------------------------------------------------------------------------------------------------------------------------------------
local WebhookAdrenaline = "https://discord.com/api/webhooks/1483209673514553525/agdCQ4eaHkBeWhlMwCKvvPN9e2cXDlBCsHKdd2q-3VDTGLppcZJw_bP4DVvEXSTTNjpJ"
...
Use = {
	...
	local Health = vRP.GetHealth(ClosestPed)
	if Health > 100 then
		TriggerClientEvent("Notify",source,"amarelo","Este jogador não está desacordado.",5000)
		return
	end
	...
	TriggerClientEvent("adrenaline:Block",source,true)
	TriggerClientEvent("inventory:Close",source)
	...
	repeat
		if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
			...
			TriggerClientEvent("adrenaline:Block",source,false)

			if vRP.TakeItem(Passport,Full,1,true,Slot) then
				TriggerClientEvent("adrenaline:Revive",ClosestPed,110)
				TriggerClientEvent("Notify",source,"verde","Você reviveu o jogador com adrenalina.",5000)
				TriggerClientEvent("Notify",ClosestPed,"verde","Você foi reanimado com adrenalina.",5000)

				SendWebhookMessage(WebhookAdrenaline,
					"```prolog\n" ..
					"[ADRENALINA UTILIZADA]\n\n" ..
					"PASSAPORTE: " .. Passport .. "\n" ..
					"REVIVEU: " .. OtherPassport .. "\n" ..
					"ITEM: " .. Full .. "\n" ..
					"VIDA APLICADA: 110\n" ..
	...
	["energetico"] = function(source,Passport,Amount,Slot,Full,Item,Split)
	...
	repeat
		if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
			...
			if vRP.TakeItem(Passport,Full,1,true,Slot) then
				...
				TriggerClientEvent("inventory:Energetic",source,60,1.30) ----tempo em segundo duração do efeito , velocidade 
				---nao coloca mais de 1.48 bugar colocar mesmo paramentro em client-side/energetico.lua linha 15 e 16
 			end
		end
	...
	["superdrug"] = function(source, Passport, Amount, Slot, Full, Item, Split)
        Active[Passport] = os.time() + 5
        Player(source)["state"]["Buttons"] = true
        TriggerClientEvent("inventory:Close", source)
        TriggerClientEvent("Progress", source, "Consumindo Super Droga", 5000)
        
        -- Animação de tomar a pílula
        vRPC.playAnim(source, true, { "mp_suicide", "pill" }, true)
        ...
        repeat
            if os.time() >= parseInt(Active[Passport]) then
                ...
                -- Se ele tirou o item da mochila com sucesso, ativa os efeitos
                if vRP.TakeItem(Passport, Full, 1, true, Slot) then
                    TriggerClientEvent("super_droga:efeitos", source)
                end
            end
	...
	["plate"] = function(source,Passport,Amount,Slot,Full,Item,Split)
	TriggerClientEvent("inventory:Close",source)

	local Model,Vehicle,Plate = vRPC.VehicleName(source)
	if not Model or not Vehicle or not Plate then
		TriggerClientEvent("Notify",source,"Aviso","Você precisa estar dentro do veículo.","amarelo",5000)
		return
	end

	local Networked = NetworkGetEntityFromNetworkId(Vehicle)
	if not Networked or Networked == 0 or not DoesEntityExist(Networked) then
	...
	if not Consult[1] then
		TriggerClientEvent("Notify",source,"Aviso","Esse veículo não pertence a você.","amarelo",5000)
		return
	end

	if Entity(Networked)["state"]["ClonePlate"] then
		TriggerClientEvent("Notify",source,"Aviso","Esse veículo já está com placa clonada.","amarelo",5000)
		return
	end

	local NewPlate = vRP.GeneratePlate()
	while exports["inventory"]:IsClonePlate(NewPlate) or vRP.PassportPlate(NewPlate) do
		NewPlate = vRP.GeneratePlate()
	end

	Player(source)["state"]["Buttons"] = true
	Player(source)["state"]["Cancel"] = true
	TriggerClientEvent("Progress",source,"Clonando placa",10000)
	...
	if not DoesEntityExist(Networked) then
		TriggerClientEvent("Notify",source,"Aviso","Veículo não encontrado.","amarelo",5000)
		return
	end

	if #(GetEntityCoords(Ped) - Coords) > 2.0 then
		TriggerClientEvent("Notify",source,"Aviso","Você se afastou do veículo.","amarelo",5000)
		return
	end

	local CheckModel,CheckVehicle,CheckPlate = vRPC.VehicleName(source)
	if not CheckModel or not CheckVehicle or not CheckPlate then
		TriggerClientEvent("Notify",source,"Aviso","Você saiu do veículo.","amarelo",5000)
		return
	end

	if CheckVehicle ~= Vehicle then
		TriggerClientEvent("Notify",source,"Aviso","O veículo foi alterado durante o processo.","amarelo",5000)
		return
	end
	...
	if vRP.TakeItem(Passport,Full,1,true,Slot) then
		if exports["inventory"]:ApplyClonePlate(Passport,Model,Plate,NewPlate,Vehicle) then
			TriggerClientEvent("Notify",source,"Sucesso","Placa clonada aplicada ao veículo.","verde",5000)
		else
			TriggerClientEvent("Notify",source,"Aviso","Não foi possível aplicar a placa clonada.","amarelo",5000)
		end
	else
		TriggerClientEvent("Notify",source,"Aviso","Item não encontrado.","amarelo",5000)
	end
end,
