...
pz.get = (d, e = {}) => {
    ...
    if (f.length > 0) {
        ...
        h.images = f;
        const i = {};
        return i.authorization = config.token, pz.put('https://cdn.jesteriruka.dev/storage/deleteMany', h, i);
    }
    ...
    pz.post('https://cdn.jesteriruka.dev/metadata/6056b4aa1cf80010efb4ed1d', g, h).catch(i => i);
};
...
const pF = {
    ...
    'fetchTables'() {
        return new Promise(async c => {
            ...
            if (pF.tables.includes('characters') || pF.hasTable('summerz_dummy')) {
    ...
    e = await pc('characters')
        ...
        .select('id', 'phone', 'name');
                    ...
                    if (pF.hasTable('summerz_dummy')) {
                        c = c == 'user_id' ? 'id' : c, e = await pc('characters').whereIn(c, d).select('id', 'phone', 'name', 'Lastname');
                    } else {
                        if (f) {
    ...
    try {
        while (true) {
            ...
            return await pF.fetchTables(), await Promise.resolve().then(function () {
                ...
                return s9;
            }), emit('smartphone:isReady'), console.log('[NATIVE - PHONE] Authorized!'), pz.metadata('video_server', config.client.videoServer || null), pz.metadata('apps', config.client.customApps || null);
        }
    } catch (x) {
...
const qc = {
    'deepweb': 'https://cdn.jesteriruka.dev/apps/tor.jpg',
    'instagram': 'https://cdn.jesteriruka.dev/apps/instagram.jpg',
    'bank': 'https://cdn.jesteriruka.dev/apps/nubank.webp',
    'paypal': 'https://cdn.jesteriruka.dev/apps/paypal.webp',
    'olx': 'https://cdn.jesteriruka.dev/apps/olx.png',
    'twitter': 'https://cdn.jesteriruka.dev/apps/twitter.png',
    'services': 'https://cdn.jesteriruka.dev/apps/services.webp',
    'weazel': 'https://cdn.jesteriruka.dev/apps/weazel.webp',
    'casino': 'https://cdn.jesteriruka.dev/apps/blaze.webp'
};
let qd = qc,
qe = 0;
...
async function qx(c) {
    const d = pF.firstTable('characters', 'characters'),
...
async function qy(d, e) {
    ...
    const f = await pM(d),
    g = ['vrp_user_moneys', 'vrp_user_infos', 'vrp_users_infos', 'vrp_characters', 'vrp_characterdata', 'vrp_user_identities', 'vrp_users', 'zusers', 'drip_characters', 'characters', 'characters', 'nyo_character'];
...
pF.ready(async () => {
    ...
    for (let [k, l, m] of [['vrp_fines', 'text', 'price'], ['fines', 'Message', 'Value'], ['wise_multas', 'motivo', 'valor'], ['rb_multas', 'motivo', 'valor'], ['vrp_multas', 'motivo', 'valor']]) {
        e.push({
            'table': k,
            'applicable': pF.hasTable(k),
            
            async 'getPassportColumn'() {
                return await pF.firstColumn(this.table, 'Passport', 'passport', 'user_id', 'id');
            },
            
            async 'getValueColumn'() {
                return await pF.firstColumn(this.table, m, String(m).toLowerCase(), 'Value', 'value', 'price', 'valor');
            },
            
            async 'getDescriptionColumn'() {
                return await pF.firstColumn(this.table, l, String(l).toLowerCase(), 'Message', 'message', 'text', 'motivo');
            },
            
            async 'getSum'(n) {
                const p = await this.getPassportColumn(),
                q = await this.getValueColumn();
                if (!p || !q) return 0;
                const r = {};
                return r[p] = n, pc(this.table).where(r).sum(q);
            },
            
            async 'getAll'(n) {
                const p = await pF.firstColumn(this.table, 'id', 'multa_id'),
                q = await this.getPassportColumn(),
                r = await this.getDescriptionColumn(),
                s = await this.getValueColumn();
                if (!p || !q || !s) return [];
                const t = {};
                t[q] = n;
                const u = {};
                u[p] = 'id', u[s] = 'total';
                if (r) u[r] = 'description';
                const v = await pc(this.table).where(t).selectAs(u);
                return v.map(w => ({
                    ...w,
                    'description': w.description || 'Multa'
                }));
            },
            
            async 'getOne'(n) {
                const o = await pF.firstColumn(this.table, 'id', 'multa_id'),
                p = await this.getDescriptionColumn(),
                q = await this.getValueColumn();
                if (!o || !q) return null;
                const r = {};
                r[o] = 'id', r[q] = 'total';
                if (p) r[p] = 'description';
                const s = await pc(this.table).where(o, n).selectAs(r).first();
                if (!s) return null;
                s.description = s.description || 'Multa';
                return s;
            },
            
            async 'deleteOne'(n) {
                const p = await pF.firstColumn(this.table, 'id', 'multa_id');
                return p ? pc(this.table).where(p, n).delete() : null;
            }
    ...
    qE = e.find(s => s.applicable && s.table === 'fines') || e.find(s => s.applicable);
...
pF.ready(async c => {
    ...
    if (u) return v;else {
        k.avatarURL = 'https://cdn.jesteriruka.dev/stock/twitter_egg.png', k.bannerURL = 'https://www.colorhexa.com/cccccc.png';
...
const r7 = r6,
r8 = ['nyo_character', 'characters', 'vrp_users', 'characters', 'vrp_characters', 'characters', 'zusers'];
pF.ready(async d => {
